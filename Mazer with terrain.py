...
def maze_maker(size=(20,20)):
    ...
    ----------
    size : (int,int)
    The size of the rectangle you can build a maze in. The default size is a 20*20 square
            ...
            if held and xw<p[0]<500+x-xw and yw<p[1]<500+y-yw:
                if not(colour==(255,0,0) and any([2 in i for i in mp])):
                    if not(colour==(0,255,0) and any([3 in i for i in mp])):
                        ...
                        if colour == (255,0,0) or colour == (0,255,0):
                            held=False
...
def maze_solver(mp, distance= lambda a,b: ((a[0]-b[0])**2+(a[1]-b[1])**2)):
    ...
    if type(distance)!= type(lambda:0):
        def distance(a,b):
            if type(mp[a[0]][a[1]])==int:
                return ((a[0]-b[0])**2+(a[1]-b[1])**2)**(1/2)-explored[now][0]
            else:
                return 0
...
walls = [[e for e in i ]for i in saved_maze]

maze_solver(walls,func)
